...
def test_subquery():
    with psql_engine.connect() as testing_connection:
        testing_connection.execute(
            insert(User).values(
                [
                    {"name": "JohnA", "account_balance": 100},
                    {"name": "JohnB", "account_balance": 101},
                    {"name": "JohnC", "account_balance": 102},
                    {"name": "JohnD", "account_balance": 103},
                    {"name": "JohnE", "account_balance": 104},
                    {"name": "JohnAB", "account_balance": 200},
                    {"name": "JohnBC", "account_balance": 201},
                    {"name": "JohnCD", "account_balance": 202},
                    {"name": "JohnDE", "account_balance": 203},
                    {"name": "JohnEF", "account_balance": 204},
                ]
            )
        )
        testing_connection.commit()
        ...
        users_over_100 = (
            select(User.name, User.account_balance)
            .where(User.account_balance > 99, User.account_balance < 300)
            .subquery()
        )
        users_over_100_with_a_in_name = select(
            users_over_100.c.name, users_over_100.c.account_balance
